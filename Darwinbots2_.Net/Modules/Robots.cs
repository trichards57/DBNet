...
internal static class Robots
{
    ...
    private static void ChangeChlr(robot rob)
    {
        ...
        {
            var newnrg = rob.nrg - (rob.chloroplasts - tmpchlr) * SimOpts.Costs[CHLRCOST] * SimOpts.Costs[COSTMULTIPLIER];
    ...
    private static double SetAimFunc(robot rob)
    {
        ...
        rob.nrg -= Math.Abs(Math.Round((diff + diff2) / 200, 3) * SimOpts.Costs[TURNCOST] * SimOpts.Costs[COSTMULTIPLIER]);
    ...
    private static void Shoot(robot rob)
    {
        ...
        if (shtype == -1 || shtype == -6)
        {
            ...
            {
                rngmultiplier = 1;
                cost = SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER] / (rob.Ties.Count + 1);
            }
            ...
            {
                multiplier = 1;
                cost = SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER] / (rob.Ties.Count + 1); //Botsareus 6/12/2014 Bug fix
            }

            if (cost > rob.nrg && cost > 2 && rob.nrg > 2 && valmode)
            {
                cost = rob.nrg;
                multiplier = Math.Log(rob.nrg / (SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER]), 2);
            }

            if (cost > rob.nrg && cost > 2 && rob.nrg > 2 && !valmode)
            {
                cost = rob.nrg;
                rngmultiplier = Math.Log(rob.nrg / (SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER]), 2);
            }
        }
        ...
        switch (shtype)
        {
            ...
            case -2:
                ...
                energyLost = value + SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER] / (rob.Ties.Count + 1);
            ...
            case -3:
                ...
                energyLost = SimOpts.Costs[SHOTCOST] * SimOpts.Costs[COSTMULTIPLIER] / (rob.Ties.Count + 1);
    ...
    private static void StoreResource(robot rob, int storeAddress, int levelAddress, double conversionRate, int costAddress, Func<robot, double> getValue, Action<robot, double> setValue, bool multiBotDiscount = false)
    {
        if (rob.nrg <= 0)
            return;
        ...
        var cost = Math.Abs(delta) * SimOpts.Costs[costAddress] * SimOpts.Costs[COSTMULTIPLIER];
    ...
    private static void Upkeep(robot rob)
    {
        ...
        //Age Cost
        var ageDelta = rob.age - SimOpts.Costs[AGECOSTSTART];
        ...
        //BODY UPKEEP
        Cost = rob.body * SimOpts.Costs[BODYUPKEEP] * SimOpts.Costs[COSTMULTIPLIER];
        rob.nrg -= Cost;

        //DNA upkeep cost
        Cost = (rob.dna.Count - 1) * SimOpts.Costs[DNACYCCOST] * SimOpts.Costs[COSTMULTIPLIER];
        rob.nrg -= Cost;
