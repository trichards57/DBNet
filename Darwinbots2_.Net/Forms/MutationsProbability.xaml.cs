...
namespace DBNet.Forms
{
    public partial class MutationsProbability : Window
    {
        ...
        public static MutationsProbability instance { set { _instance = null; } get { return _instance ?? (_instance = new MutationsProbability()); } }

        public static void Load()
        {
            if (_instance == null) { dynamic A = MutationsProbability.instance; }
        }
        ...
        private void Form_QueryUnload(int Cancel_UNUSED, int UnloadMode_UNUSED)
        {
            if (!optionsform.CurrSpec == 50)
            {
                ...
                if (dir(path) == "")
                {
                    outpath = MDIForm1.instance.MainDir + "\\Robots\\" + extractexactname(TmpOpts.Specie(optionsform.CurrSpec).Name) + ".mrate";
                }
                Save_mrates(TmpOpts.Specie(optionsform.CurrSpec).Mutables, outpath);
            }
        }

        private void Lower_LostFocus(object sender, RoutedEventArgs e)
        {
            Lower_LostFocus();
        }

        private void Lower_LostFocus(int Index)
        {
            Mean(Index).text = (val(Lower(Index).text) + val(Upper(Index).text)) / 2;
            RevalueStdDev(Index);
        }

        private void Mean_Change(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            Mean_Change();
        }

        private void Mean_Change(int Index)
        {
        ...
        private void MutTypeEnabled_Click(object sender, RoutedEventArgs e)
        {
            MutTypeEnabled_Click();
        }
        ...
        private void Probs_Change(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            Probs_Change();
        }

        private void Probs_Change(int Index)
        {
            if (Index == 0)
            {
        ...
        private void RevalueStdDev(int Index)
        {
        ...
        private void SetupDelta()
        {
            ...
            Text2.text = "The mutation rates of a bot are allowed to change " + "slowly over time.  This change in mutation rates can include " + "Delta Mutations as well.  Theoretically, it may be possible for " + "a bot to figure its own optimal mutation rate.";
        ...
        private void StdDev_Change(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            StdDev_Change();
        }

        private void StdDev_Change(int Index)
        {
        ...
        private void TypeOption_Click(int Index)
        {
            int value = 0;
        ...
        private void Upper_LostFocus(object sender, RoutedEventArgs e)
        {
            Upper_LostFocus();
        }

        private void Upper_LostFocus(int Index)
        {
            Mean(Index).text = (val(Lower(Index).text) + val(Upper(Index).text)) / 2;
            RevalueStdDev(Index);
        }
